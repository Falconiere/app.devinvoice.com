...
import  { type Business, businessSchema } from "@/database/services/business/types";
...
const useBusinessFormController = (options?:UseBusinessFormController) => {
  ...
  useEffect(() => {
    if (business) {
      for (const [key, value] of Object.entries(business)) {
        ...
        setValue(key as keyof Business, value);
      }
    }
  }, [business, setValue]);
  ...
  return {form, onSubmit, isPending: isLoading || isPending, errors, register };
}
